...
/// View model backing `SavedPaymentMethodView`.
///
...
@MainActor
final class SavedPaymentMethodViewModel: ObservableObject {
    ...
    private let onResult: (SavedPaymentMethodResult) -> Void
    ...
    func onAppear() {
        apiClient?.sendAnalyticsEvent(UIComponentsAnalytics.savedPaymentMethodPresented)
        if request.showCreditMessage, style.creditMessaging.enabled {
            apiClient?.sendAnalyticsEvent(UIComponentsAnalytics.creditMessagingPresented)
        }
        ...
        //   - no FI and no email / failure  → .hidden  (+ savedPaymentMethodFetchFailed)
        // Until then the component stays in `.loading` (skeleton) on-device; previews
        // seed concrete states.
    }

    func editTapped() {
        apiClient?.sendAnalyticsEvent(UIComponentsAnalytics.savedPaymentMethodEditSelected)
    ...
    func learnMoreTapped() {
        apiClient?.sendAnalyticsEvent(UIComponentsAnalytics.creditMessagingSelected)
